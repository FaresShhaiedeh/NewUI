...
class TrackingService {
  ...
  /// Calculate upcoming stops for a specific bus
  /// Returns list of upcoming stops with estimated arrival times
  BusStopInfo? getUpcomingStops(String busId) {
    try {
      // Find the bus
      ...
      // Use all bus stops if line doesn't have specific stops
      final stopsToUse = busLine.stops.isEmpty ? _busStops : busLine.stops;

      if (stopsToUse.isEmpty) {
        debugPrint('[UpcomingStops] No stops available');
        return null;
      }

      debugPrint(
        '[UpcomingStops] Processing ${stopsToUse.length} stops for bus $busId',
      );
      ...
      for (int i = 0; i < stopsToUse.length; i++) {
        ...
        // A stop is "passed" if the bus is more than 50 meters away AND
        // the next stop is closer than this stop
        bool isPassed = false;
        ...
        if (i < stopsToUse.length - 1) {
          ...
          // If current stop is far (>50m) and next stop is closer, consider it passed
        ...
        // Average bus speed in urban areas: 20 km/h = 5.56 m/s
      ...
      // Sort upcoming by distance (closest first)
      upcomingStops.sort(
